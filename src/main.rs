...
enum CacheError {
    ...
    SemaphoreError(#[from] tokio::sync::SemaphorePermitError),
...
trait EvictionPolicy<K> {
    fn evict(&mut self, cache: &mut HashMap<K, CacheEntry>) -> Option<K>;
}
...
impl DiskCache {
    // Creates a new DiskCache instance with the given directory and capacity
...
fn generate_nonce() -> Result<Nonce, CacheError> {
    let mut nonce = [0u8; 12];
    OsRng.fill_bytes(&mut nonce);
    Ok(Nonce::from_slice(&nonce))
}

fn encrypt(data: String, nonce: &Nonce) -> Result<Vec<u8>, CacheError> {
    let key = load_encryption_key()?;
    let cipher = Aes256Gcm::new(&key);
    cipher.encrypt(nonce, data.as_ref()).map_err(|e| CacheError::EncryptionError(e.to_string()))
}

// Utility function to hash a value
...
async fn main() -> Result<(), CacheError> {
    ...
    if !nix::unistd::Uid::effective().is_root() {
        println!("The application is not running with root privileges. The service file will be created in the current directory.");
        let local_path = format!("./{}.service", service_name);
        let mut file = File::create(&local_path)?;
        file.write_all(service_file_content.as_bytes())?;

        println!("Service file created at: {}", local_path);
        println!("Please move this file to /etc/systemd/system/ with root privileges.");
    } else {
